...
SES$region.n <- match(SES$region.n, sort(unique(SES$region.n)))

gens <- read_csv("Genlengths.csv") |> filter(region != "Fraser")
sst.gen <- read_csv("SST_genlengths.csv") |> filter(region != "Fraser")
