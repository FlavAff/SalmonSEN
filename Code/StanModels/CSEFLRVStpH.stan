...
data {
  ...
  // Hatchery data
  int<lower=0> N_Hobs_total;            // Total number of observed hatchery values
  vector[N_Hobs_total] log_Hobs;       // Observed hatchery values (flattened)
  array[N_Hobs_total] int<lower=1, upper=N_loc> Hobs_location_id; // Location ID for each hatchery observation
  array[N_Hobs_total] int<lower=1, upper=N_year_max> Hobs_year_index; // Year index for each hatchery observation
    
  // Province-wide Recreational License Observations
  int<lower=0> N_Lrecobs_total;           // Number of observations for province-wide Lrec
  vector[N_Lrecobs_total] log_Lrecobs; // Observed values for Lrec_total
  array[N_Lrecobs_total] int<lower=1, upper=N_year_max> Lrecobs_year_index; // Year index for each recreational observation
...
parameters {
  ...
  // Hyperparameters for log_arec (location-specific effect: allocation & participation)
  real mu_log_arec;
  real<lower=0> sigma_log_arec;
  // Hyperparameters for log_erec (location-specific effort per active rec fisher)
  real mu_log_erec;
  ...
  real mu_log_H;
  real<lower=0> sigma_log_H;
  // Parameters for latent hatchery release
  array[N_loc] vector[N_year_max] log_H;
  // Hyperparameters for log_Erec
  real mu_log_Lrec;
  real<lower=0> sigma_log_Lrec;
  vector[N_year_max] log_Lrec;
  ...
  vector[N_loc] log_arec_raw;   
  vector[N_loc] log_erec_raw;   
  ...
  vector<lower=0>[N_loc] sigma_h;              // Observation error for hatcheries
  real<lower=0> sigma_lrec;       // Recreational effort observation error for each location
...
transformed parameters {
  ...
  vector[N_loc] log_arec;
  vector[N_loc] log_erec;
  ...
  gamma = mu_gamma + sigma_gamma * gamma_raw;
  ...
  log_arec = mu_log_arec + sigma_log_arec * log_arec_raw;
  log_erec = mu_log_erec + sigma_log_erec * log_erec_raw;
  ...
  for (l in 1:N_loc) {
    ...
    log_Erec[l] = rep_vector(0.0, N_year_max);
    ...
    for (y in 1:N_year_max) {
      ...
      log_Erec[l][y] = log_Lrec[y] + log_arec[l] + log_erec[l];
    ...
    for (t in (genlength[l] + 1):N_year_max) {
      ...
      log_R[l][t] = log_S[l][t - genlength[l]] + alpha[l] - beta[l] * exp(log_S[l][t - genlength[l]]) + gamma[l] * exp(log_H[l][t - genlength[l]]) + log_R_Perror[l][t];
...
model {
  ...
  mu_log_arec ~ normal(0, 2);
  sigma_log_arec ~ exponential(1);
  mu_log_erec ~ normal(1, 1.5);
  sigma_log_erec ~ exponential(1);
  
  // Hyperpriors for missing data with no regressors
  mu_log_L ~ normal(6, 1);
  sigma_log_L ~ exponential(1);
  mu_log_Lrec ~ normal(7, 1);
  sigma_log_Lrec ~ exponential(1);
  mu_log_H ~ normal(6,2);
  sigma_log_H ~ exponential(1);
  ...
  log_arec_raw ~ std_normal();
  log_erec_raw ~ std_normal();

  // Priors for all latent values - missing data with no regressors
  for (l in 1:N_loc) {
    log_L[l] ~ normal(mu_log_L, sigma_log_L);
  }
  log_Lrec ~ normal(mu_log_Lrec, sigma_log_Lrec);
  for (l in 1:N_loc) {
    log_H[l] ~ normal(mu_log_H, sigma_log_H);
  }
  ...
  sigma_lrec ~ exponential(4);
  sigma_h ~ exponential(4);
  ...
  // Likelihood for observed hatcheries
  for (n_h in 1:N_Hobs_total) {
    log_Hobs[n_h] ~ normal(log_H[Hobs_location_id[n_h]][Hobs_year_index[n_h]], sigma_h[Hobs_location_id[n_h]]);
  }
  ...
  for (n in 1:N_Lrecobs_total) {
    log_Lrecobs[n] ~ normal(log_Lrec[Lrecobs_year_index[n]], sigma_lrec);
  }
}
...
generated quantities {
  ...
  array[N_loc] vector[N_year_max] log_H_rep;
  vector[N_year_max] log_Lrec_rep;
  ...
  array[N_loc] vector[N_year_max] log_lik_hatcheries;
  vector[N_year_max] log_lik_stamps;
  ...
  for (l in 1:N_loc) {
    ...
    log_H_rep[l] = rep_vector(0.0, N_year_max);
    ...
    log_lik_hatcheries[l] = rep_vector(0.0, N_year_max);
    ...
    for (y in 1:N_year_max) {
        ...
        log_H_rep[l][y] = normal_rng(log_H[l][y], sigma_h[l]);
    }
    ...
    // Log likelihood for hatcheries
    for (n_h in 1:N_Hobs_total) {
      if (Hobs_location_id[n_h] == l && Hobs_year_index[n_h] <= N_year_max) {
        log_lik_hatcheries[l][Hobs_year_index[n_h]] = normal_lpdf(log_Hobs[n_h] | log_H[l][Hobs_year_index[n_h]], sigma_h[l]);
      }
    }
  ...
  // log_Lrec is not an array so fill it on its own
  for (y in 1:N_year_max){
    log_Lrec_rep[y] = normal_rng(log_Lrec[y], sigma_lrec);
  }
  for (n in 1:N_Lrecobs_total) {
    if (Lrecobs_year_index[n] <= N_year_max) {
      log_lik_stamps[Lrecobs_year_index[n]] = normal_lpdf(log_Lrecobs[n] | log_Lrec[Lrecobs_year_index[n]], sigma_lrec);
    }
  }
}
